...
local/make_data_dir.py \
   ...
   $DEV_DATA_DIR \
   $DEV_WAV_DIR \
   $DIHARD_DEV_DIR/data/multichannel/sad
...
local/make_data_dir.py \
   ...
   $EVAL_DATA_DIR \
   $EVAL_WAV_DIR \
   $DIHARD_EVAL_DIR/data/multichannel/sad
utils/fix_data_dir.sh $EVAL_DATA_DIR

# Diarize.
echo "Diarizing..."
./alltracksrun.sh --tracknum 3 --plda_path exp/xvector_nnet_1a/plda_track3

# Extract dev/eval RTTM files.
echo "Extracting RTTM files..."
DEV_RTTM_DIR=$THIS_DIR/rttm_dev
local/split_rttm.py \
    exp/xvector_nnet_1a/xvectors_dihard_dev_2019_track3/plda_scores/rttm $DEV_RTTM_DIR
EVAL_RTTM_DIR=$THIS_DIR/rttm_eval
local/split_rttm.py \
    exp/xvector_nnet_1a/xvectors_dihard_eval_2019_track3/plda_scores/rttm $EVAL_RTTM_DIR
