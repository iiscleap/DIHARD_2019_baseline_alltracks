...
# Denoise WAV.
DEV_WAV_DEN_DIR=exp/dihard_dev_2019_single_chan_wav_den
echo "Denoising DEV audio..."
if [ -d $DEV_WAV_DEN_DIR ]; then
    echo "Found existing denoised WAV at:"
    echo ""
    echo "    $DEV_WAV_DEN_DIR"
    echo ""
else
    local/run_denoising.sh $DEV_WAV_DIR $DEV_WAV_DEN_DIR
fi
EVAL_WAV_DEN_DIR=exp/dihard_eval_2019_single_chan_wav_den
echo "Denoising EVAL audio..."
if [ -d $EVAL_WAV_DEN_DIR ]; then
    echo "Found existing denoised WAV at:"
    echo ""
    echo "    $EVAL_WAV_DEN_DIR"
    echo ""
else
    local/run_denoising.sh $EVAL_WAV_DIR $EVAL_WAV_DEN_DIR
fi

# Perform VAD.
DEV_VAD_DIR=exp/dihard_dev_2019_single_chan_vad_den
...
if [ -d $DEV_VAD_DIR ]; then
    ...
    local/run_vad.sh $DEV_WAV_DEN_DIR $DEV_VAD_DIR $NJOBS
fi
EVAL_VAD_DIR=exp/dihard_eval_2019_single_chan_vad_den
...
if [ -d $EVAL_VAD_DIR ]; then
    ...
    local/run_vad.sh $EVAL_WAV_DEN_DIR $EVAL_VAD_DIR $NJOBS
fi

# Prepare data directory for DEV set.
echo "Preparing data directory for DEV set..."
DEV_DATA_DIR=data/dihard_dev_2019_track2_den
rm -fr $DEV_DATA_DIR
...
EVAL_DATA_DIR=data/dihard_eval_2019_track2_den
rm -fr $EVAL_DATA_DIR
local/make_data_dir.py \
   --audio_ext	'.flac'	\
   $EVAL_DATA_DIR \
...
# Diarize.
echo "Diarizing..."
./alltracksrun.sh --tracknum 2_den --plda_path exp/xvector_nnet_1a/plda_track2 --njobs $NJOBS

# Extract dev/eval RTTM files.
echo "Extracting RTTM files..."
DEV_RTTM_DIR=$THIS_DIR/rttm_dev
local/split_rttm.py \
    exp/xvector_nnet_1a/xvectors_dihard_dev_2019_track2_den/plda_scores/rttm $DEV_RTTM_DIR
EVAL_RTTM_DIR=$THIS_DIR/rttm_eval
local/split_rttm.py \
    exp/xvector_nnet_1a/xvectors_dihard_eval_2019_track2_den/plda_scores/rttm $EVAL_RTTM_DIR
